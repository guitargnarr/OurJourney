...
function AppEnhanced() {
  ...
  const [newNote, setNewNote] = useState({ message: '', for_partner: true })
  ...
  const createNote = async () => {
    ...
    try {
      await axios.post(`${API_ENDPOINT}/entries`, {
        type: 'note',
        title: newNote.for_partner ? 'For Partner 💕' : 'For Matt 💙',
        content: newNote.message,
        author: newNote.for_partner ? 'matt' : 'partner'
      })
      setNewNote({ message: '', for_partner: true })
      loadNotes()
    } catch (error) {
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <input
                    type="radio"
                    checked={newNote.for_partner}
                    onChange={() => setNewNote({...newNote, for_partner: true})}
                  />
                  For Partner 💕
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <input
                    type="radio"
                    checked={!newNote.for_partner}
                    onChange={() => setNewNote({...newNote, for_partner: false})}
                  />
                  For Matt 💙
                </label>
              </div>
